...
webD.get('https://www.amazon.in/s?i=sporting&bbn=4730577031&rh=n%3A4730577031%2Cp_89%3AGarmin&dc&qid=1644590353&rnid=3837712031&ref=sr_pg_1')	
for i in range(4,27):			
	print(i)
	try:
		j=i-3
...
pages = webD.find_elements(By.CLASS_NAME, "s-pagination-button")
no_of_pages_after_first = len(pages)
for k in range (0, no_of_pages_after_first):
	url = find_element(By.TAG_NAME,"a").get_attribute('href')[k]
	webD.get(url)
	for x in range(4,27):			
		print(x)
		try:
			y=x-3
			image_link = webD.find_elements(By.CLASS_NAME, "s-image")[y].get_attribute('src')
			print(image_link)
			page_data = webD.find_elements(By.CLASS_NAME, "sg-col-inner")[x]
			details_link = page_data.find_element(By.TAG_NAME,"a").get_attribute('href')
			print(details_link)
			# Open a new window
			webD.execute_script("window.open('');")
